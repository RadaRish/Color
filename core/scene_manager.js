  ...
  async switchToScene(sceneId) {
    ...
    try {
      ...
      if (this.hotspotManager) {
        ...
        console.log('🔍 Маркеры для восстановления:', sceneHotspots.map(h => ({ id: h.id, title: h.title, sceneId: h.sceneId, color: h.color, type: h.type })));

        sceneHotspots.forEach(hotspot => {
          console.log('🎯 Восстанавливаем маркер:', hotspot.id, 'для сцены:', hotspot.sceneId, 'с цветом:', hotspot.color, 'тип:', hotspot.type);
